...
public class ToolConfigPrerequisiteChecker {
    ...
    @ConfigProperty(name = "gateforge.cluster-domain", defaultValue = DEFAULT_CLUSTER_DOMAIN)
    ...
    public List<MigrationPrerequisite> fromConfig(String targetClusterId) {
        ...
        if (!connected) {
            result.add(toolConfig(
                    "gateforge-cluster-api",
                    "Kubernetes API connection",
                    "GateForge needs a working Kubernetes API client to apply resources to the target cluster.",
                    "unknown"));
        }
        ...
        if (!"local".equals(effectiveId)) {
            ...
            if (cluster == null || !cluster.enabled()) {
                result.add(toolConfig(
                        "gateforge-target-cluster",
                        "Target cluster configuration",
                        "The selected target cluster must be registered and enabled in GateForge.",
                        "unknown"));
            }
        }
        ...
        if (DEFAULT_CLUSTER_DOMAIN.equals(clusterDomain)) {
            result.add(toolConfig(
                    "gateforge-cluster-domain",
                    "Cluster domain",
                    "Set gateforge.cluster-domain (CLUSTER_DOMAIN) to your cluster ingress domain before applying Routes and test URLs.",
                    "unknown"));
        }
